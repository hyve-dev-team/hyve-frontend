...
const ConversationChatHeader = ({ contact, backTo = "/landlord/chats" }) => {
  const avatar = contact?.avatarSrc || defaultProfileImage;
  const name = contact?.name || "Prospective Tenant";
  const phone = contact?.phone || "";
  const isOnline = Boolean(contact?.isOnline);
  const subtitle =
    contact?.subtitle || (isOnline ? "Online" : phone || "Prospective Tenant");
    ...
    <header className="sticky top-0 z-20 flex items-center justify-between px-3 py-3 bg-white border-b sm:px-6 lg:px-8 border-[#0000000D] shadow-sm">
      <div className="flex items-center justify-between w-full">
        <div className="flex items-center space-x-2.5 sm:space-x-3.5 min-w-0">
          ...
          <Link
            to={backTo}
            className="p-1.5 -ml-1 text-[#3D3129]/70 hover:text-primary hover:bg-black/5 rounded-full transition-colors flex items-center justify-center shrink-0 cursor-pointer"
