...
const ScheduleTour = () => {
    ...
    // Apartment details
    const { apartment, isLoading: isApartmentLoading } = useFetchApartment(apartmentID);
    ...
    const [tenantName, setTenantName] = useState(
        cachedUser ? `${cachedUser.firstName || ''} ${cachedUser.lastName || ''}`.trim() : ''
    );
    ...
    useEffect(() => {
        if (!inspection || inspection.status === 'COMPLETED' || inspection.status === 'CANCELLED') return;
            ...
            try {
                ...
                if (updated && updated.status !== inspection.status) {
                    ...
                    if (updated.status === 'ACCEPTED') {
                        hyveSuccess("Tour Confirmed! 🎉", `${updated.assignedContactName} accepted your inspection.`);
                    } else if (updated.status === 'PROPOSED_NEW_TIME') {
                        hyveSuccess("Agent Proposed New Time", `${updated.assignedContactName} suggested ${updated.proposedDate} at ${updated.proposedTime}.`);
                    }
                }
            } catch {
                // silent
    ...
    useEffect(() => {
        if (inspection && inspection.tourDate && inspection.tourTime) {
            const timer = setInterval(() => {
                ...
                const scheduledDateTime = new Date(`${inspection.tourDate}T${inspection.tourTime}`).getTime();
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setInspection(data);
            hyveSuccess("Tour Scheduled!", `Inspection request created for ${tourDate} at ${tourTime}.`);
            ...
            hyveError("Booking Failed", err?.message || "Could not schedule tour. Please try again.");
        } finally {
    ...
    const handleAcceptProposedTime = async () => {
        ...
        try {
            ...
            setInspection(updated);
            hyveSuccess("Time Accepted", `Tour confirmed for ${updated.tourDate} at ${updated.tourTime}!`);
        } catch (err) {
    ...
    const handleConfirmInspectionFeePayment = async () => {
        ...
        try {
            ...
            setInspection(updated);
            hyveSuccess("Agent Contact Unlocked!", "Phone number and direct WhatsApp messaging are now available.");
            if (updated.whatsappUrl) {
                window.open(updated.whatsappUrl, "_blank");
            }
        } catch (err) {
            hyveError("Payment Failed", err?.message || "Could not complete fee payment.");
        } finally {
    ...
    const cleanAgentPhone = (inspection?.inspectionFeePaid && inspection?.assignedContactPhone && !inspection.assignedContactPhone.includes("Locked"))
    ...
    return (
        <div className='page-wrapper'>
                        ...
                        {!inspection ? (
                            <div className='bg-white rounded-3xl border border-[#EAEAEA] p-6 sm:p-10 shadow-sm'>
                                <div className="text-center max-w-md mx-auto mb-8">
                                    ...
                                    <p className="text-xs sm:text-sm text-[#6B7280] mt-1.5 leading-relaxed">
                                        Select your preferred viewing date and time. An automated inspection request and viewing link will be sent directly to the property caretaker via WhatsApp.
                                    </p>
                                </div>
                                ...
                                <form onSubmit={handleSubmit} className="max-w-lg mx-auto space-y-4">
                                    ...
                                    >
                                        ...
                                        {isSubmitting ? "Generating Viewing Link..." : "Book Tour & Notify Agent on WhatsApp"}
                                    </button>

                                    <p className="text-[11px] text-center text-[#9CA3AF] mt-3">
                                        By booking, a unique inspection link is created allowing the caretaker or property owner to confirm immediately.
                                    </p>
                                </form>
                            </div>
                        ) : (
                            ...
                            <div className="space-y-6">
                                    ...
                                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-[#F3F4F6]">
                                        <div>
                                            <div className="flex items-center gap-2 mb-1.5">
                                                {inspection.status === 'ACCEPTED' ? (
                                                    <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold bg-[#ECFDF5] text-[#059669] border border-[#A7F3D0]">
                                                        <CheckCircle2 className="w-3.5 h-3.5" /> Confirmed by Agent
                                                    </span>
                                                    ...
                                                    <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold bg-[#FFFBEB] text-[#D97706] border border-[#FDE68A]">
                                                        <RotateCcw className="w-3.5 h-3.5" /> Agent Proposed New Time
                                                    </span>
                                                    ...
                                                    <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold bg-[#FEF2F2] text-[#DC2626] border border-[#FECACA]">
                                                        <XCircle className="w-3.5 h-3.5" /> Agent Unavailable
                                                    </span>
                                                ) : (
                                                    <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold bg-primary/10 text-primary border border-primary/20">
                                                        <Clock className="w-3.5 h-3.5" /> Tour Scheduled — Pending Agent Reply
                                                    </span>
                                                )}
                                            </div>
                                            ...
                                            <p className="text-xs sm:text-sm text-[#6B7280] mt-1">
                                                Scheduled for <span className="font-semibold text-[#1F2937]">{inspection.tourDate}</span> at <span className="font-semibold text-[#1F2937]">{inspection.tourTime}</span>
                                            </p>
                                        </div>
                                            ...
                                            <div className="flex items-start gap-3">
                                                ...
                                                <div>
                                                    <h3 className="text-xs sm:text-sm font-bold text-[#92400E]">
                                                        {inspection.assignedContactName} proposed an alternative time:
                                                    </h3>
                                                    <p className="text-xs text-[#B45309] mt-0.5 font-medium">
                                                        📅 {inspection.proposedDate} at {inspection.proposedTime}
                                                        {inspection.agentNotes && ` — "${inspection.agentNotes}"`}
                                                    </p>
                                                </div>
                                            </div>
                                            ...
                                            <div className="flex items-center gap-3.5">
                                                ...
                                                <div>
                                                    ...
                                                    <p className="text-xs text-[#6B7280] font-mono mt-0.5 flex items-center gap-1">
                                                        <span>Contact:</span>
                                                        {inspection.inspectionFeePaid ? (
                                                            <span className="text-[#1F2937] font-semibold">{inspection.assignedContactPhone}</span>
                                                        ) : (
                                                            <span className="text-amber-700 font-medium flex items-center gap-1">
                                                                <Lock className="w-3 h-3" /> {inspection.assignedContactPhone || "••••••••••• (Locked)"}
                                                            </span>
                                                        )}
                                                    </p>
                                                </div>
                                            ...
                                            <div className="mt-4 p-3.5 rounded-xl bg-amber-50/80 border border-amber-200 flex items-start gap-2.5 text-xs text-amber-900">
                                                ...
                                                <div className="flex-1">
                                                    ...
                                                    <p className="text-amber-700 text-[11px] mt-0.5">
                                                        Pay the ₦5,000 inspection fee to reveal the caretaker's direct phone number and send them an instant WhatsApp viewing invite.
                                                    </p>
                                                    <button
                                                        type="button"
                                                        onClick={() => setIsPaymentModalOpen(true)}
                                ...
                                {!countdownElapsed ? (
                                    ...
                                    <PostScheduleTour
                                        reversePayOnSubmit={(e) => { e.preventDefault(); setIsReversePayment(true); }}
                                        confirmPayOnSubmit={(e) => { e.preventDefault(); setIsConfirmPayment(true); }}
                                    />
                                )}
                            </div>
                        )}
            ...
            <PayInspectionModal
                ...
                queue={{
                    id: inspection?.id,
                    property: inspection?.propertyTitle || apartment?.title || "Apartment",
                }}
